...
#             relayed to us via the GitHub project page linked below.
#
...
$$ |  $$\ $$  _$$<        $$ |  $$ |$$ |  $$ | \____$$\   $$ |$$\ $$ |$$ |  $$ |$$ |  $$ |
\$$$$$$  |$$ | \$$\       $$ |  $$ |\$$$$$$  |$$$$$$$  |  \$$$$  |$$ |$$ |  $$ |\$$$$$$$ |
 \______/ \__|  \__|      \__|  \__| \______/ \_______/    \____/ \__|\__|  \__| \____$$ |
                                                                                $$\   $$ |
                                                                                \$$$$$$  |
                                                                                 \______/ '
echo '----------------------------------------------------------------------------------------------------------'
...
if [ ! -z "$PRINT_HELP" ]; then
	echo -e
	echo -e "Usage: ./benchmark.ck-hosting.com [-flags]"
	echo -e "       curl -sL benchmark.ck-hosting.com | bash"
	echo -e "       curl -sL benchmark.ck-hosting.com | bash -s -- -flags"
	echo -e "       wget -qO- benchmark.ck-hosting.com | bash"
	echo -e "       wget -qO- benchmark.ck-hosting.com | bash -s -- -flags"
	echo -e
	echo -e "Flags:"
	...
	echo -e "       -j : print jsonified benchmark results at conclusion of test"
	echo -e "       -w <filename> : write jsonified benchmark results to disk using file name provided"
	echo -e "       -s <url> : send jsonified benchmark results to URL"
	echo -e
	...
	[[ $JSON = *s* ]] && echo -e "       sharing json benchmark results to $JSON_SEND" 
	echo -e
	echo -e "Exiting..."
	exit 0
fi
...
if [ ! -z $JSON ]; then
	...
	JSON_RESULT='{"version":"'$BENCHMARK_VERSION'","time":"'$TIME_START'","os":{"arch":"'$ARCH'","distro":"'$DISTRO'","kernel":"'$KERNEL'",'
...
# if the skip disk flag was set, skip the disk performance test, otherwise test disk performance
	...
	if [[ -f "$ZFSCHECK" ]];then
		...
		if [[ $warning -eq 1 ]];then
			echo -en "\nWarning! You are running the benchmark on a ZFS Filesystem and your disk space is too low for the fio test. Your test results will be inaccurate. You need at least $mul_spa GB free in order to complete this test accurately."
		fi
	fi
...
function calculate_time_taken() {
    ...
    if [ ${time_taken} -gt 60 ]; then
        ...
        echo "Benchmark completed in ${min} min ${sec} sec"
    else
        echo "Benchmark completed in ${time_taken} sec"
    fi
}
